...
// Fetch current profile details for use in the Profile Settings modal.
$profileStmt = $conn->prepare("SELECT * FROM main_admin_accounts WHERE username = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        ...
        switch ($action) {
            ...
            case 'update_profile':
                // Process Profile Settings update for the main admin.
                ...
                // Process cropped profile image if provided
                $profile_photo_path = null;
                if (isset($_POST['croppedProfileImage']) && !empty($_POST['croppedProfileImage'])) {
                    $croppedData = $_POST['croppedProfileImage'];
                    $croppedData = preg_replace('/^data:image\/\w+;base64,/', '', $croppedData);
                    $croppedData = str_replace(' ', '+', $croppedData);
                    $decoded = base64_decode($croppedData);
                    if ($decoded === false) {
                        $errors[] = "Error decoding profile image.";
                    } else {
                        $upload_dir = 'uploads/profile_photos/';
                        if (!is_dir($upload_dir)) {
                            mkdir($upload_dir, 0755, true);
                        }
                        $newFileName = $upload_dir . 'profile_' . $username . '_' . time() . '.png';
                        if (file_put_contents($newFileName, $decoded) === false) {
                ...
                if ($profile_photo_path) {
                    ...
                    $stmt->bind_param("ssiss", $full_name, $email, $contact_number_int, $profile_photo_path, $username);
                } else {
                    $stmt = $conn->prepare("UPDATE main_admin_accounts SET full_name = ?, email = ?, contact_number = ? WHERE username = ?");
                    $stmt->bind_param("ssis", $full_name, $email, $contact_number_int, $username);
                }
                if ($stmt->execute()) {
...
$office_admins = $conn->query("
    SELECT accounts.*, offices.office_name 
    FROM accounts 
...
<div class="sidebar">
    ...
    <ul class="nav flex-column">
        ...
        <li class="nav-item">
            <a class="nav-link" href="#" data-toggle="modal" data-target="#addOfficeModal">
                <i class="bi bi-plus-circle"></i> Add Office
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="#office-summary">
        ...
        <li class="nav-item">
    <a class="nav-link" href="#" data-toggle="modal" data-target="#profileSettingsModal">
...
<div class="main-content">
    ...
    <?php
    // Date filtering and recent activity feed
    $dateRanges = [
     ...
     <div class="recent-activity">
      <div class="section-title d-flex justify-content-between align-items-center" id="recent-activity">
        ...
        <form method="GET" class="d-inline-block">
          <label for="date _range" class="me-2 small">Filter:</label>
...
<div class="modal fade" id="addOfficeModal" tabindex="-1" aria-labelledby="addOfficeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="add OfficeModalLabel">Add Office</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-body">
                <form id="profileSettingsForm">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="currentUsername" value="<?php echo htmlspecialchars($_SESSION['username']); ?>" readonly>
                    </div>
                    <div class="form-group">
                        <label for="currentEmail">Current Email</label>
                        <input type="email" class="form-control" id="currentEmail" name="currentEmail" readonly>
                    </div>
                    <div class="form-group">
...
<script>
    ...
    const servicesChart = new Chart(document.getElementById('servicesChart'), {
        ...
        options: {
            responsive: true,
            plugins: { legend: { position: 'top' }, title: { display: true, text: 'Content Distribution' } }
        }
    });
    ...
    const visitorsLineChart = new Chart(document.getElementById('visitorsLineChart'), {
        ...
        options: {
            responsive: true,
            plugins: { legend: { position: 'top' }, title: { display: true, text: 'Visitor Movement (Last 7 Days)' } }
        }
    });
    ...
    // -- Enhanced Profile Settings: Preview and Crop --
    ...
    var croppedProfileImageInput = document.getElementById('croppedProfileImage');
    var cropper;
    ...
    profilePhotoInput.addEventListener('change', function(e) {
        ...
        if (files && files.length > 0) {
            ...
            reader.onload = function(event) {
                ...
                cropper = new Cropper(profilePhotoPreview, {
                    aspectRatio: 1,
                    viewMode: 1,
                    autoCropArea: 1
                });
    ...
    document.getElementById('profileSettings Form').addEventListener('submit', function(e) {
    e.preventDefault();
    if (cropper) {
        var canvas = cropper.getCroppedCanvas({ width: 300, height: 300 });
        if (canvas) {
            croppedProfileImageInput.value = canvas.toDataURL('image/png');
        }
    }
