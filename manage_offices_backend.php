...
if ($active_section) {
    ...
    elseif ($type == 'gallery') {
        ...
        // (1) Handle media upload (file upload or video link)
        if (isset($_POST['action']) && $_POST['action'] === 'upload_media') {
            $media_title = sanitize_input($conn, $_POST['media_title'] ?? '');
            $caption = sanitize_input($conn, $_POST['caption'] ?? '');
            
            // Option A: Upload via video link
            if (!empty($_POST['video_link'])) {
                $video_link = sanitize_input($conn, $_POST['video_link']);
                $query = "INSERT INTO gallery_content (office_id, section_id, video_link, caption, title, media_type) 
                          VALUES ({$active_section['office_id']}, {$active_section['id']}, '$video_link', '$caption', '$media_title', 'video_link')";
                if ($conn->query($query)) {
                    $galleryMsg = "Video link uploaded successfully.";
                } else {
                    $galleryMsg = "Failed to upload video link: " . $conn->error;
                }
            }
            // Option B: File upload (image or video file)
            elseif (isset($_FILES['media']) && $_FILES['media']['error'] === 0) {
                $filename = time() . '_' . basename($_FILES['media']['name']);
                $targetFile = 'uploads/gallery/' . $filename;
                if (move_uploaded_file($_FILES['media']['tmp_name'], $targetFile)) {
                    $ext = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));
                    $video_extensions = ['mp4', 'avi', 'mov', 'wmv', 'flv', 'webm'];
                    $media_type = in_array($ext, $video_extensions) ? 'video' : 'image';
                    
                    $query = "INSERT INTO gallery_content (office_id, section_id, file_path, caption, title, media_type) 
                              VALUES ({$active_section['office_id']}, {$active_section['id']}, '$targetFile', '$caption', '$media_title', '$media_type')";
                    if ($conn->query($query)) {
                        $galleryMsg = ($media_type === 'video') ? "Video file uploaded successfully." : "Image uploaded successfully.";
                    } else {
                        $galleryMsg = "Failed to upload media file: " . $conn->error;
                    }
                } else {
                    $galleryMsg = "Error moving uploaded file.";
                }
            } else {
                $galleryMsg = "No file or video link provided, or a file upload error occurred.";
            }
        }
    
        // (2) Handle media deletion based on office permission
        if (isset($_GET['delete_media'])) {
            $mediaId = intval($_GET['delete_media']);
            $query = "SELECT file_path, video_link, media_type FROM gallery_content WHERE id = $mediaId AND office_id = {$active_section['office_id']}";
            $result = $conn->query($query);
            if ($result && $result->num_rows > 0) {
                $data = $result->fetch_assoc();
                if (in_array($data['media_type'], ['image', 'video']) && file_exists($data['file_path'])) {
                    unlink($data['file_path']);
                }
                // Video links don't have local files to delete
                $conn->query("DELETE FROM gallery_content WHERE id = $mediaId AND office_id = {$active_section['office_id']}");
            }
            header("Location: manage_offices_interface.php?office_id={$active_section['office_id']}&section_id={$active_section['id']}");
            exit();
        }
    
        // (3) Handle media editing (updating title, caption, and optionally replacing the media)
        if (isset($_POST['action']) && $_POST['action'] === 'edit_media') {
            $media_id = intval($_POST['media_id']);
            $media_title = sanitize_input($conn, $_POST['media_title'] ?? '');
            $caption = sanitize_input($conn, $_POST['caption'] ?? '');
            
            $query = "SELECT file_path, video_link, media_type FROM gallery_content WHERE id = $media_id AND office_id = {$active_section['office_id']}";
            $result = $conn->query($query);
            if ($result && $result->num_rows > 0) {
                $data = $result->fetch_assoc();
                ...
                if (isset($_FILES['media']) && $_FILES['media']['error'] === 0) {
                    if (in_array($current_media_type, ['image', 'video']) && file_exists($current_file_path)) {
                    ...
                    if (move_uploaded_file($_FILES['media']['tmp_name'], $targetFile)) {
                        ...
                        $stmt = $conn->prepare("UPDATE gallery_content SET title = ?, caption = ?, file_path = ?, video_link = NULL, media_type = ? 
                                                WHERE id = ? AND office_id = {$active_section['office_id']}");
                        $stmt->bind_param("sssii", $media_title, $caption, $targetFile, $media_type, $media_id);
                        $stmt->execute();
                        $galleryMsg = ($media_type === 'video') ? "Video file updated successfully." : "Image updated successfully.";
                    } else {
                        $galleryMsg = "Error uploading new media file.";
                    }
                }
                // Option B: Update using a new video link
                elseif (!empty($_POST['video_link'])) {
                    if (in_array($current_media_type, ['image', 'video']) && file_exists($current_file_path)) {
                        unlink($current_file_path);
                    }
                    $video_link = sanitize_input($conn, $_POST['video_link']);
                    $stmt = $conn->prepare("UPDATE gallery_content SET title = ?, caption = ?, video_link = ?, file_path = NULL, media_type = ? 
                                            WHERE id = ? AND office_id = {$active_section['office_id']}");
                    $media_type = 'video_link';
                    $stmt->bind_param("sssii", $media_title, $caption, $video_link, $media_type, $media_id);
                    $stmt->execute();
                    $galleryMsg = "Video link updated successfully.";
                }
                // Option C: Update title and caption only
                else {
                    $stmt = $conn->prepare("UPDATE gallery_content SET title = ?, caption = ? 
                                            WHERE id = ? AND office_id = {$active_section['office_id']}");
                    $stmt->bind_param("ssi", $media_title, $caption, $media_id);
                    $stmt->execute();
                    $galleryMsg = "Media details updated successfully.";
                }
            } else {
                $galleryMsg = "Media not found for editing.";
            }
        }
    }
...
elseif ($type == 'services') {
    if (isset($_POST['action']) && $_POST['action'] == 'upload_service') {
        ...
        if ($wordCount > 30) {
            ...
            // Include office_id along with section_id, caption, and title
            $office_id = intval($active_section['office_id']);
            $section_id = intval($active_section['id']);
            $conn->query("INSERT INTO services_content (section_id, office_id, caption, title) VALUES ($section_id, $office_id, '$combined', '$service_title')");
    ...
    if (isset($_GET['delete_service'])) {
        ...
        $conn->query("DELETE FROM services_content WHERE id = $svcId AND section_id = $section_id");
        header("Location: manage_offices_interface.php?office_id={$active_section['office_id']}&section_id={$active_section['id']}");
        exit();
    }
}
