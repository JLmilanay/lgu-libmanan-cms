...
// Fetch user details including office_id and status from the accounts table
$sql = "SELECT office_id, status FROM accounts WHERE id = ?";
...
// Check if the account status is active (case-insensitive); if not, call the custom HTML file.
if (strtoupper(trim($user['status'])) !== 'ACTIVE') {
    include('account_disable_message.html');  // Make sure this file exists in the proper directory.
    exit();
}
...
.table-scrollable tr {
    ...
    table-layout: fixed;
        ...
        /* Styles for the content management area */
        .content-management-area {
            ...
            margin: 20px auto;
            /* Center the box with auto margins */
            border-radius: 5px;
            ...
            width: 90%;
            /* Width of the box relative to the viewport */
        }
    ...
    <div class="sidenav">
        ...
        <nav class="nav-links">
            <?php if ($selected_office) { ?>
                ...
                <ul>
                    <li>
                        <a href="#" data-toggle="modal" data-target="#uploadLogoModal">
                            <i class="fas fa-upload"></i> Upload Office Logo
                        </a>
                    </li>
                    <li>
                        <a href="#" data-toggle="modal" data-target="#addSectionModal">
                            <i class="fas fa-plus-circle"></i> Add Section
                        </a>
                    </li>
                    <li>
                        <a href="#" data-toggle="modal" data-target="#updateAppearanceModal">
                            <i class="fas fa-paint-brush"></i> Update Section Background
                        </a>
                    </li>
                    <li class="nav-item">
            ...
            <div class="col-md-8">
                <?php if ($selected_office) { ?>
                    ...
                    <div class="modal fade" id="profileModal" tabindex="-1" aria-labelledby="profileModalLabel"
                        aria-describedby="profileModalDescription" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered">
                            <!-- Removed modal-dialog-scrollable class -->
                            <div class="modal-content">
                                <div class="modal-header">
                                ...
                                <div class="modal-body">
                                    ...
                                    <p id="profileModalDescription" class="text-muted small">
                                        Update your profile information below. Changes will be saved when you click "Save
                                        changes."
                                    </p>

                                    <!-- Loading Spinner (hidden by default; toggle with JS upon form submission) -->
                                    <div id="modal-spinner" class="d-none text-center mb-3">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <!-- Save changes button submits the form; assumes the form has id="profile-settings-form" -->
                                    <button type="submit" form="profile-settings-form" class="btn btn-primary">
                                        Save changes
                                    </button>
                                </div>
                            </div>
                    ...
                    <!-- Optional JavaScript: Auto-focus the first interactive element when the modal is shown -->
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            var profileModalEl = document.getElementById('profileModal');
                            profileModalEl.addEventListener('shown.bs.modal', function () {
                                var firstFormElement = profileModalEl.querySelector('input, select, textarea, button');
                                if (firstFormElement) {
                    ...
                    <div class="content-management-area" style="border: 2px dashed #ccc; padding: 20px; margin: 20px 0;">
                        <?php if ($active_section) { ?>
                            <h4>Manage <?php echo ucfirst(str_replace('_', ' ', $active_section['section_type'])); ?> Section
                            </h4>
                            <hr>
                            <?php
                            if (isset($heroMsg))
                            ...
                            switch ($active_section['section_type']) {
                                ...
                                case 'gallery':
                                    // Define a helper function to parse YouTube video ID for thumbnails
                                    function parse_youtube_id($url)
                                    {
                                        ...
                                        if (isset($parsed_url['query'])) {
                                            ...
                                            return $query_params['v'] ?? ''; // Extract 'v' parameter for YouTube video ID
                                        }

                                        // For shortened YouTube URLs (e.g., youtu.be/VIDEO_ID)
                                        if (isset($parsed_url['host']) && $parsed_url['host'] === 'youtu.be') {
                                            return trim($parsed_url['path'], '/'); // Return the path as the ID
                                        }

                                        // Return empty string if video ID cannot be extracted
                                        return '';
                                    }

                                    // Button to trigger the modal for uploading a new media
                                    echo "<button type='button' class='btn btn-primary' data-toggle='modal' data-target='#uploadMediaModal'>Upload New Media</button>";

                                    // Modal for uploading a new media
                                    echo "<div class='modal fade' id='uploadMediaModal' tabindex='-1' role='dialog' aria-labelledby='uploadMediaModalLabel' aria-hidden='true'>
                                        <div class='modal-dialog' role='document'>
                                            <div class='modal-content'>
                                                <div class='modal-header'>
                                                    <h5 class='modal-title' id='uploadMediaModalLabel'>Upload New Media</h5>
                                                    <button type='button' class='close' data-dismiss='modal' aria-label='Close'>
                                                        <span aria-hidden='true'>&times;</span>
                                                    </button>
                                                </div>
                                    ...
                                    // Display gallery media in a table
                                    $resMedia = $conn->query("SELECT * FROM gallery_content WHERE section_id = {$active_section['id']} ORDER BY created_at DESC");
                                    echo "<hr>";
                                    if ($resMedia && $resMedia->num_rows > 0) {
                                        echo "<table class='table table-bordered table-scrollable'>";                                        echo "<thead><tr><th>Media</th><th>Title</th><th>Caption</th><th>Actions</th></tr></thead><tbody>";
                                        while ($media = $resMedia->fetch_assoc()) {
                                            echo "<tr>";
                                            if ($media['media_type'] == 'image') {
                                                echo "<td><img src='" . htmlspecialchars($media['file_path']) . "' class='media-img' style='max-width:100px;' data-toggle='modal' data-target='#mediaModal-{$media['id']}'></td>";
                                            } elseif ($media['media_type'] == 'video') {
                                                echo "<td><video width='100' controls><source src='" . htmlspecialchars($media['file_path']) . "' type='video/mp4'>Your browser does not support the video tag.</video></td>";
                                            } elseif ($media['media_type'] == 'video_link') {
                                                // Display video link thumbnail
                                                $thumbnail = "https://img.youtube.com/vi/" . parse_youtube_id($media['video_link']) . "/0.jpg";
                                                echo "<td><img src='" . htmlspecialchars($thumbnail) . "' alt='Video Thumbnail' class='media-img' style='max-width:100px;'></td>";
                                            }
                                            echo "<td>" . htmlspecialchars($media['title']) . "</td>";
                                            echo "<td>" . htmlspecialchars($media['caption']) . "</td>";
                                            echo "<td>
                                                <a href='" . htmlspecialchars($media['media_type'] == 'video_link' ? $media['video_link'] : $media['file_path']) . "' target='_blank' class='btn btn-sm btn-info'>View Fullscreen</a>
                                                <a href='#' class='btn btn-sm btn-warning' data-toggle='modal' data-target='#editMediaModal-{$media['id']}'>Edit</a>
                                                <a href='manage_offices_interface.php?office_id={$selected_office['id']}&section_id={$active_section['id']}&delete_media={$media['id']}' class='btn btn-sm btn-danger'>Delete</a>
                                              </td>";
                                            echo "</tr>";
                                        }
                                        echo "</tbody></table>";
                                    ...
                                    echo "<script>
                                        document.addEventListener('DOMContentLoaded', function() {
                                            // Ensure file upload is displayed by default
                                        ...
                                        document.getElementById('media_type').addEventListener('change', function() {
                                            ...
                                            document.getElementById('mediaFile').style.display = (mediaType === 'image' || mediaType === 'video') ? 'block' : 'none';
                                            document.getElementById('videoLink').style.display = (mediaType === 'video_link') ? 'block' : 'none';
                                        });
                                    </script>";
                                    break;
                                ...
                                .addEventListener('change', function() {
                                    ...
                                    if ($aboutContents && $aboutContents->num_rows > 0) {
                                        echo "<table class='table table-bordered table-scrollable'>";                                        echo "<thead><tr><th>Content Type</th><th>Title</th><th>Content</th><th>Actions</th></tr></thead><tbody>";
                                ...
                                case 'services':
                                    ...
                                    if ($resSvc && $resSvc->num_rows > 0) {
                                        echo "<table class='table table-bordered table-scrollable'>";                                        echo "<thead><tr><th>Title</th><th>Description</th><th>Actions</th></tr></thead><tbody>";
...
-labelledby='editServiceModalLabel-{$svc['id']}' aria-hidden='true'>
                                ...
                                <script>
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        echo "<table class='table table-bordered table-scrollable'>";
